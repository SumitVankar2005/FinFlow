...
-- Reset auto-increment sequences so future inserts don't collide with the
-- explicit IDs used above (Postgres SERIAL doesn't auto-track manual IDs).
SELECT setval(pg_get_serial_sequence('Users', 'user_id'), COALESCE((SELECT MAX(user_id) FROM Users), 1));
SELECT setval(pg_get_serial_sequence('Accounts', 'account_id'), COALESCE((SELECT MAX(account_id) FROM Accounts), 1));
SELECT setval(pg_get_serial_sequence('Income', 'income_id'), COALESCE((SELECT MAX(income_id) FROM Income), 1));
SELECT setval(pg_get_serial_sequence('Expenses', 'expense_id'), COALESCE((SELECT MAX(expense_id) FROM Expenses), 1));
SELECT setval(pg_get_serial_sequence('Budgets', 'budget_id'), COALESCE((SELECT MAX(budget_id) FROM Budgets), 1));
SELECT setval(pg_get_serial_sequence('Subscriptions', 'subscription_id'), COALESCE((SELECT MAX(subscription_id) FROM Subscriptions), 1));
SELECT setval(pg_get_serial_sequence('Subscription_Flags', 'flag_id'), COALESCE((SELECT MAX(flag_id) FROM Subscription_Flags), 1));
SELECT setval(pg_get_serial_sequence('Investments', 'investment_id'), COALESCE((SELECT MAX(investment_id) FROM Investments), 1));
SELECT setval(pg_get_serial_sequence('Loans_Purchases', 'loan_id'), COALESCE((SELECT MAX(loan_id) FROM Loans_Purchases), 1));
SELECT setval(pg_get_serial_sequence('Flag_Assignments', 'assignment_id'), COALESCE((SELECT MAX(assignment_id) FROM Flag_Assignments), 1));
